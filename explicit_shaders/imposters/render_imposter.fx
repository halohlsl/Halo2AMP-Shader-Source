...
#if DX_VERSION == 11
	//The vertex buffer for the impostor uses _rasterizer_vertex_format_byte_vector4d_normalized( DXGI_FORMAT_R8G8B8A8_UNORM ) which was stored endian swaped on the XBOX.  
	//We have legacy data stored this way and new data being generated this way, so the fastest most stable fix is to swizzle the data here....
	///I feel dirty.  
	OUT.diffuse= vertex.diffuse.abgr * vertex.diffuse.abgr;
	OUT.ambient= vertex.ambient.abgr * vertex.ambient.abgr;
	OUT.specular_shininess.rgb = vertex.specular_shininess.agb * vertex.specular_shininess.agb;
	OUT.specular_shininess.a = vertex.specular_shininess.r;

	OUT.change_colors_of_diffuse= vertex.change_colors_of_diffuse.abgr * vertex.change_colors_of_diffuse.abgr;
	OUT.change_colors_of_specular= vertex.change_colors_of_specular.abgr * vertex.change_colors_of_specular.abgr;
#else
