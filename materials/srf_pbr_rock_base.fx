...
#if defined(VERT_ROUGHNESS)

	DECLARE_FLOAT_WITH_DEFAULT(macro_darkening_multiplier, "Vert Roughness Darkening", "", 0, 1, float(1));
	#include "used_float.fxh"

	DECLARE_RGB_COLOR_WITH_DEFAULT(macro_darkening_tint,		"Vert Roughness Tint", "", float3(1,1,1));
	#include "used_float3.fxh"
	
	DECLARE_FLOAT_WITH_DEFAULT(vert_rough_intensity, "Vert Roughness Intensity", "", 0, 1, float(1));
	#include "used_float.fxh"

	DECLARE_FLOAT_WITH_DEFAULT(water_specular, "Water Specular Coefficient", "", 0, 1, float(0.04));
	#include "used_float.fxh"
	
	#if defined(WATER_NORMALS)
		DECLARE_SAMPLER( water_map, "Water Map", "Water Map", "shaders/default_bitmaps/bitmaps/default_normal.tif");
		#include "next_texture.fxh"
	#endif 
...
// Details ------------------------------------------------------------------------------------------------------------------------
#if defined( DETAIL_NORMALS )

DECLARE_SAMPLER(ao_detail_map,		"Detail AO Map", "", "shaders/default_bitmaps/bitmaps/color_white.tif");
#include "next_texture.fxh"

DECLARE_SAMPLER(normal_detail_map,		"Detail Normal Map", "", "shaders/default_bitmaps/bitmaps/default_normal.tif");
#include "next_texture.fxh"

DECLARE_FLOAT_WITH_DEFAULT(detail_normal_blend_intensity,		"Detail Normal Map Influence", "", 0, 1, float(1.0));
#include "used_float.fxh"

// Second Details ------------------------------------------------------------------------------------------------------------------
#if defined( DETAIL_NORMALS_2 )

DECLARE_SAMPLER(ao_detail_2_map,		"Second Detail AO Map", "", "shaders/default_bitmaps/bitmaps/color_white.tif");
#include "next_texture.fxh"

DECLARE_SAMPLER(normal_detail_2_map,		"Second Detail Normal Map", "", "shaders/default_bitmaps/bitmaps/default_normal.tif");
#include "next_texture.fxh"

DECLARE_FLOAT_WITH_DEFAULT(detail_normal_blend_intensity_2,		"Detail Normal Map 2 Influence", "", 0, 1, float(1.0));
#include "used_float.fxh"
...
#endif //#if defined( DETAIL_NORMALS_2 )

#endif //DETAIL_NORMALS
...
DECLARE_FLOAT_WITH_DEFAULT(world_mask_min,		"World Space Mask Min", "", 0, 1, float(1));
#include "used_float.fxh"

DECLARE_FLOAT_WITH_DEFAULT(world_mask_max,		"World Space Mask Max", "", 0, 1, float(1));
#include "used_float.fxh"
...
// Grime ---------------------------------------------------------------------------------------------------------------------------
#if defined( USE_GRIME_LAYER )

DECLARE_SAMPLER(grime_map,		"Grime Map", "", "shaders/default_bitmaps/bitmaps/color_white.tif");
#include "next_texture.fxh"

DECLARE_FLOAT_WITH_DEFAULT(grime_intensity,		"Grime Intensity", "", 0, 1, float(1));
#include "used_float.fxh"

#endif //#if defined( USE_GRIME_LAYER )

// Moss Snow -------------------------------------------------------------------------------------------------------------------------

#if defined( USE_UPFACING_LAYER )
...
DECLARE_FLOAT_WITH_DEFAULT(upfacing_roughness,		"Up Facing Roughness", "", 0, 1, float(0.5));
#include "used_float.fxh"
...
#endif //#if defined( USE_UPFACING_LAYER )

#ifdef USE_SPARKLE

	DECLARE_SAMPLER(sparkle_map,		"Sparkle Map", "", "shaders/default_bitmaps/bitmaps/color_white.tif");
	#include "next_texture.fxh"
            ...
            inout s_shader_data shader_data)
{
	...
	#if defined( DETAIL_NORMALS )
	{
		...
		detail_normal.xy *= detail_normal_blend_intensity;
		...
		#if defined( DETAIL_NORMALS_2 )
		{
			...
			detail_2_normal.xy *= detail_normal_blend_intensity_2;
			base_normal += lerp(detail_2_normal, detail_normal, detail_lerp) - float3(0,0,1);			
		}
		#else
		{
			base_normal += detail_normal - float3(0,0,1);
		}
		#endif //#if defined( DETAIL_NORMALS_2 )

		base_normal = normalize(base_normal);
	}
	#endif //#if defined( DETAIL_NORMALS )
	
	#if defined( WATER_NORMALS ) && defined(VERT_ROUGHNESS)
	{
		// Composite detail normal map onto the base normal map
		float2 water_uv = transform_texcoord(uv2, water_map_transform);
		float3 water_normal = sample_2d_normal_approx(water_map, water_uv);
		base_normal = lerp(  base_normal, base_normal + water_normal - float3(0,0,1), shader_data.common.vertexColor.a);	
	}
	#endif
	...
	#if defined( USE_UPFACING_LAYER )
	{
		float maskBias = mask_bias;
			...
			up_mask = pow(saturate((  (lerp( maskDirection,maskDirection*(shader_data.combo_map.b),mask_combo_influence)) ) * mask_intensity), mask_power) ;
				...
				up_mask = saturate(up_mask + scaled_vert_color);
			#endif 
    ...
    {// Sample color map.
		...
		#if defined( USE_UPFACING_LAYER )
		{
			...
			shader_data.common.albedo.rgb = lerp(shader_data.common.albedo.rgb, lerp(shader_data.common.albedo.rgb, shader_data.common.albedo.rgb * macro_darkening_tint, macro_darkening_multiplier), shader_data.common.vertexColor.a); 
		...
		#if !defined(USE_UPFACING_LAYER)
			shader_data.common.shaderValues.y = detail_cavity;
		#endif 
            ...
            inout s_shader_data shader_data) 
{
	...
	#ifdef VERT_ROUGHNESS
		float vert_rough = shader_data.common.shaderValues.x;
	#endif
	
	#if !defined(USE_UPFACING_LAYER)
		detail_cavity = shader_data.common.shaderValues.y;
	#endif 
	
	#if !defined(VERT_ROUGHNESS)
		detail_cavity = shader_data.common.shaderValues.x;
	...
	float4 combo 	= shader_data.combo_map;
 	...
 	#ifdef USE_SPARKLE
		float2 sparkle_map_uv = transform_texcoord(uv, sparkle_map_transform);
		float sparkle = sample2D(sparkle_map, sparkle_map_uv).r * sparkle_intensity;
	...
	float rough = roughness * combo.a;
		...
		rough = lerp(rough , saturate(rough+ vert_rough_intensity), vert_rough); 
	#endif 
	#if defined( USE_UPFACING_LAYER )
	...
	#ifdef VERT_ROUGHNESS
		specular_color = lerp(specular_color, water_specular , vert_rough);
	#endif 
	
	#ifdef USE_SPARKLE
		specular_color = saturate(specular_color + sparkle);
		rough = saturate(rough + sparkle);
	#endif 
