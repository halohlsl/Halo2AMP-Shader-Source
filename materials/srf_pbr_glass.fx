...
DECLARE_RGB_COLOR_WITH_DEFAULT(parallax_tint,		"Parallax Tint", "", float3(1,1,1));
#include "used_float3.fxh"
            ...
            inout s_shader_data shader_data)
{
    ...
    {// Sample color map.
		...
		float2 offset = parallax_texcoord(uv, parallax_depth, viewTS, pixel_shader_input);

	    float2 color_map_uv = transform_texcoord(uv, color_map_transform);
		shader_data.common.albedo = sample2DGamma(color_map, color_map_uv + offset);
            ...
            inout s_shader_data shader_data) 
{
	...
	float3 color_map =  parallax_tint * alpha;//the albedo color will come through the specular if we are metalic
	
    out_color.rgb =  ( color_map * diffuse.rgb ) + ( diffuse.rgb * reflection ) + specular;
	float3 rgb2lum = float3(0.30, 0.59, 0.11);
	out_color.a += dot(( diffuse.rgb * reflection ) + specular, rgb2lum);
