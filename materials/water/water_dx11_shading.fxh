			   	...
			   	out float wave_choppiness_ratio)
{
	...
	float2 slope     = sample3D(wave_slope_array, Convert3DTextureCoordToTextureArray(wave_slope_array,texcoord)).xy;
	float2 slope_aux = sample3D(wave_slope_array, Convert3DTextureCoordToTextureArray(wave_slope_array,texcoord_aux)).xy;
...
/*	if (interaction)
	...
	if (refraction)
	{
		...
		// TODO remove fixed screen size
		float2 position_ss =  INTERPOLATORS.position.xy / float2(1920,1080);
