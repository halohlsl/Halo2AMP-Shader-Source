...
DECLARE_SAMPLER( color_map, "Uber Color Map", "Uber Color Map", "shaders/default_bitmaps/bitmaps/default_diff.tif");
#include "next_texture.fxh"

//DECLARE_SAMPLER( color_map, "Macro Color Map", "Macro Color Map", "shaders/default_bitmaps/bitmaps/default_diff.tif");
//#include "next_texture.fxh"

//50% grey used instead of macro color map
DECLARE_RGB_COLOR_WITH_DEFAULT(macroColor,	"Macro Color", "", float3(0.5,0.5,0.5));
			...
			
{
	...
	// sample the uber color map
	    float2 uberColor_uv   = transform_texcoord(uv, color_map_transform);
        float3 uberColor = sample2DGamma(color_map, uberColor_uv).rgb;
