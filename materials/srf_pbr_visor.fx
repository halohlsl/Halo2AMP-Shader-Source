            ...
            inout s_shader_data shader_data) 
{
...
#ifndef cgfx
	float fresnel_visor = pow(shader_data.common.shaderValues.x, front_color_power);

	// Override the front and rim colors with the change colors if valid
	float3 front_color_visor = ps_material_object_parameters[2];
	float3 rim_color_visor = ps_material_object_parameters[3];

	albedo.rgb *= lerp(rim_color_visor, front_color_visor, fresnel_visor);
#else
   ...
   float3 iridescence = lerp(rim_color_cc, front_color_cc, s_ndotv ).rgb * dot(rgb2lum, clearcoat_reflection);  
