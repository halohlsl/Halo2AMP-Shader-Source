...
#ifdef USE_DETAIL_LAYER
    DECLARE_SAMPLER( detail_normal_map, "Detail Normal Map", "", "shaders/default_bitmaps/bitmaps/default_normal.tif");
    #include "next_texture.fxh"

    DECLARE_SAMPLER( detail_color_map, "Detail Albedo Map", "", "shaders/default_bitmaps/bitmaps/color_white.tif");
    #include "next_texture.fxh"
#endif 
			...
			
{
	...
	normal = mul(normalize(normal), shader_data.common.tangent_frame);
	...
	albedo.rgb = albedo.rgb * albedo_tint;
    #ifdef USE_DETAIL_LAYER
        const float DETAIL_MULTIPLIER = 4.59479f;		// 4.59479f == 2 ^ 2.2  (sRGB gamma)
        // Layer in detail color
        float2 detail_color_map_uv = transform_texcoord(uv, detail_color_map_transform);
        float4 color_detail = sample2DGamma(detail_color_map, detail_color_map_uv);
        color_detail.rgb *= DETAIL_MULTIPLIER;

        albedo.rgb *= color_detail;
	...
	out_color.rgb =  lerp((color_map * diffuse), fog_color, final_fog_value);
	 shader_data.common.albedo.rgb = out_color.rgb; 
...
float4 pixel_lighting(
	...
	inout s_shader_data shader_data) 
{
 	return float4(shader_data.common.albedo.rgb, 1);
}
