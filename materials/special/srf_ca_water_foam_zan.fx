...
DECLARE_SAMPLER( distortion_map, "Distortion Map", "Distortion Map", "shaders/default_bitmaps/bitmaps/default_normal.tif")
#include "next_texture.fxh"
DECLARE_FLOAT_WITH_DEFAULT(distortion_intensity,		"Distortion Intensity", "", 0, 1, float(1.0));
            ...
            inout s_shader_data shader_data)
{
    ...
    {// Sample color map.
	    float2 distortion_map_uv 	  = transform_texcoord(uv, distortion_map_transform);
		float2 distortion = sample_2d_normal_approx(distortion_map, distortion_map_uv).xy;
		distortion *= distortion_intensity;
	
        float2 color_map_uv 	  = transform_texcoord(uv + distortion, color_map_transform);
		float4 color = sample2DGamma(color_map, color_map_uv);
		
		float2 color_map2_uv 	  = transform_texcoord(uv + distortion, color_map2_transform);
		float4 color2 = sample2DGamma(color_map2, color_map2_uv);
		
		//Take largest color and alpha value;
		color = max(color, color2);

		shader_data.common.albedo = color;
