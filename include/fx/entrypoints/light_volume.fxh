...
LightVolumeInterpolatedValues DoSomeBungieStuff(in LightVolumeVertex input)
{
	...
	billboardBasis[1] = normalize(cross(cameraToProfile, vs_lightVolumeOverallState.direction));
...
LightVolumeInterpolatedValues Compute3DFXVolumeValues(in LightVolumeVertex input)
{
	...
	// Basis is facing camera, but rotated based on light volume direction
	float2x3 billboardBasis;
	billboardBasis[1] = normalize(cross(cameraToVolume, vs_lightVolumeOverallState.direction));
	...
	volumeBasis[1] = normalize(cross(volumeBasis[0], abs(volumeBasis[0].z < .99f) ? float3(0, 0, 1) : float3(0, 1, 0)));
